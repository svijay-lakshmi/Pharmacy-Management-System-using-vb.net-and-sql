...
Public Class userb7
    ...
    Private Sub Button8_Click(sender As Object, e As EventArgs) Handles Button8.Click
        Dim command As New SqlCommand("INSERT INTO [orders] (product_id, product_name, unit_price, quantity) VALUES (@product_id, @product_name, @unit_price, @quantity)", connection)
        command.Parameters.AddWithValue("@product_id", TextBox1.Text)
        command.Parameters.AddWithValue("@product_name", TextBox2.Text)
        command.Parameters.AddWithValue("@unit_price", TextBox3.Text)
    ...
    Private Sub Button10_Click(sender As Object, e As EventArgs) Handles Button10.Click
        Try
            Dim command As New SqlCommand("DELETE FROM [orders] WHERE product_id = @product_id", connection)
            command.Parameters.AddWithValue("@product_id", TextBox1.Text)
            connection.Open()
            If command.ExecuteNonQuery() = 1 Then
    ...
    Private Sub DataGridView2_CellContentClick(sender As Object, e As DataGridViewCellEventArgs) Handles DataGridView2.CellContentClick
        If e.RowIndex >= 0 Then
            ...
            TextBox3.Text = selectedRow.Cells("unit_price").Value.ToString()
    ...
    Private Sub Button11_Click(sender As Object, e As EventArgs) Handles Button11.Click
        ...
        command.Parameters.AddWithValue("@Net_amount", TextBox8.Text)
        ...
        For Each row As DataGridViewRow In DataGridView2.Rows
            If Not row.IsNewRow Then
                ...
                If quantityCell.Value IsNot Nothing AndAlso Integer.TryParse(quantityCell.Value.ToString(), quantity) Then
                    ...
                    If Not Double.TryParse(row.Cells("unit_price").Value.ToString(), unit_price) Then
                        MessageBox.Show("Invalid unit price in order. Please enter a valid number.")
                        Return
                    End If
                    Dim netAmount As Double = quantity * unit_price
